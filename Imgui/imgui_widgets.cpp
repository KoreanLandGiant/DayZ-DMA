...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(157 / 255.0f, 173 / 255.0f, 241 / 255.0f, it_anim->second.selected_rect), 3.0f);
    if (*v)
    {
        const float check_sz = ImMin(total_bb.GetWidth(), total_bb.GetHeight());
        const float pad = ImMax(1.0f, (float)(int)(check_sz / 6.0f));
        ImVec2 checkPos = total_bb.Min + ImVec2(pad, pad);
        ImU32 checkColor = GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 1.00f));
        float checkMarkSize = check_sz - pad * 2.0f;

        RenderCheckMark(checkPos, checkColor, checkMarkSize);
    }
