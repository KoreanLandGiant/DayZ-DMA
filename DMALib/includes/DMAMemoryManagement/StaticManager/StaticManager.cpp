...
DMAMem::MODULE_DUMP DMAMem::StaticManager::getModule(DWORD pid, std::string moduleName, bool forceUpdate) {
	...
	BOOL result = VMMDLL_Map_GetModuleFromNameU(vmmManager->getVmm(), pid, LPSTR(moduleName.c_str()), &pModuleEntry, 0);

	DMAMem::MODULE_DUMP dumpEntry;
	dumpEntry.pvmEntry = pModuleEntry;
	dumpEntry.dump = nullptr;

	if (!result || !pModuleEntry) {
		std::cerr << " [ ! ] Failed to find module: " << moduleName << std::endl;
		dllMap[pair] = dumpEntry;
		return dumpEntry;
	}

	std::cout << " [ + ] Found " << moduleName << " at 0x" << std::hex << pModuleEntry->vaBase
	          << " (size: " << std::dec << pModuleEntry->cbImageSize << " bytes)" << std::endl;

	// Only dump the module if cbImageSize is reasonable (needed for sigScan)
	if (pModuleEntry->cbImageSize > 0 && pModuleEntry->cbImageSize < 200 * 1024 * 1024) {
		...
		if (success) {
			...
			std::cerr << " [ ! ] Error Dumping " << moduleName << " (read failed)" << std::endl;
			dumpEntry.dump = nullptr;
		}
	} else {
		std::cout << " [ * ] Skipping full dump of " << moduleName << " (too large or zero size)" << std::endl;
	}
