...
namespace DMAMem {
	...
	public:
			...
			BOOL writeMemory(DWORD pid, QWORD remoteAddress, void* source, int size);
			template <typename T> T readMemoryDirect(DWORD pid, QWORD remoteAddress, ULONG64 flags = VMMDLL_FLAG_NOCACHE);
			template <typename T> BOOL writeMemoryDirect(DWORD pid, QWORD remoteAddress, T value);
	...
	template<typename T>
	inline BOOL VmmManager::writeMemoryDirect(DWORD pid, QWORD remoteAddress, T value)
	{
		return VMMDLL_MemWrite(this->getVmm(), pid, remoteAddress, (PBYTE)&value, sizeof(T));
	}
}
